...
std::pair<value_t, node_array_t>
monte_carlo_minimizer::minimize(set_function_t f, const node_array_t &ground_set)
{
    value_t min_val_so_far = MAX_VALUE;
    node_array_t best_so_far = ground_set;

    for (int i = 0; i < num_samples_; i++)
    {
        node_array_t cur_sample = draw_sample(ground_set);
...
node_array_t monte_carlo_minimizer::draw_sample(const node_array_t &ground_set)
{
    node_array_t sample;
